...
$(function () {
    function dynamic() {
        ...
        const new_table = table_html.DataTable({
            "ajax": {
                "url": "<?= base_url()?>kelas/materi/ajax_data_materi_kelas_sub/",
                "data": {
                    materi: $('#materi_id').val(),
                },
                "type": 'POST'
            },
            "processing": true,
    ...
    $("#fmain").submit(function (ev) {
        ...
        $.ajax({
            method: 'post',
            url: '<?= base_url() ?>kelas/materi/' + ($("#id").val() == "" ? 'insert_sub' : 'update_sub'),
            data: form,
            cache: false,
            contentType: false,
            processData: false,
    ...
    $('#OkCheck').click(() => {
        ...
        $.ajax({
            method: 'post',
            url: '<?= base_url() ?>kelas/materi/delete_sub',
            data: {
                id: id
            }
        }).done((data) => {
    ...
    $("#no_urut").change(function () {
        no_urut_get_check($("#materi_id").val(), this.value)
    })
})
...
const Ubah = (datas) => {
    ...
    materi_id_current = data.kelas_id;
    $('#nama').val(data.nama);
...
function no_urut_get_new(materi_id) {
    if (!check_no_urut) {
        return;
    }
    $.ajax({
        method: 'post',
        url: '<?= base_url() ?>kelas/materi/noUrut_sub',
        data: {
            materi_id: materi_id
        }
    }).done((data) => {
...
function no_urut_get_check(materi_id, no) {

    if (!check_no_urut || (no_urut_current == no && materi_id_current == materi_id)) {
        return;
    }
    $.ajax({
        method: 'post',
        url: '<?= base_url() ?>kelas/materi/cekNoUrut_sub',
        data: {
            materi_id: materi_id,
            no: no,
        }
